...
// Removed unhash-helper import - using original approach with plainId
...
export const getSecret = function (
  ...
  try {
    seed =
      path.basename(configFile) + (process.env.SEED || id.machineIdSync(true));
  } catch (err: any) {
    throw new Error(
      `consider using SEED environment because machineId can't be found - error: ${err.message}`
    );
  }
  const ivLength = 16;
  ...
  if (pw) {
    if (pw.includes("process.")) {
      // password based on external reference e.g. environment or json file
      ...
      if (pw.constructor === String && pw.includes(processEnv)) {
        ...
        try {
          ...
          pwclear = undefined;
        } // can't read external configuration file
        ...
        try {
          ...
          try {
            ...
            pwclear = objProp(
              jContent,
              `${pluginName}.${dotNotationAttr}`,
              null
            );
          } catch (err) {
            pwclear = undefined;
          } // can't JSON parse external file
        } catch (err) {
          pwclear = undefined;
        } // can't read external configuration file
      } else pwclear = undefined;
    } else {
      // password based on local configuration file
      try {
        // decrypt
        ...
        const decipher = crypto.createDecipheriv(
          "aes-128-cbc",
          Buffer.from(seed.substr(-ivLength)),
          iv
        );
        ...
        // password considered as cleartext and needs to be encrypted and written back to file
        pwclear = pw;
        const iv = crypto.randomBytes(ivLength);
        const cipher = crypto.createCipheriv(
          "aes-128-cbc",
          Buffer.from(seed.substr(-ivLength), "utf8"),
          iv
        );
...
export const timestamp = function () {
  // new Date() do not handle current timezone
  const pad = (n: number) => {
    return n < 10 ? "0" + n : n;
  };
  const d = new Date();
...
const objProp = function (obj: Record<string, any>, prop: string, val: any) {
  // return obj value based on json dot notation formatted prop
...
export const copyObj = (o: any): any => {
  // deep copy/clone faster than JSON.parse(JSON.stringify(o))
  let v, key;
...
export const extendObj = (obj: any, src: any) => {
  // copy src content into obj
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (el.type) {
          if (Object.prototype.hasOwnProperty.call(el, "value")) {
            const a = src[key].filter(
              (o) => o.type === el.type && o.value === el.value
            );
            if (a.length === 1) {
              arr.splice(i, 1);
              i -= 1;
          ...
          if (typeof obj[key][keySub] === "object") {
            for (const keySubSub in obj[key][keySub]) {
              if (
                src[key] &&
                src[key][keySub] &&
                src[key][keySub][keySubSub] === obj[key][keySub][keySubSub]
              ) {
                delete obj[key][keySub][keySubSub];
              }
            }
            if (Object.keys(obj[key][keySub]).length === 0)
...
export const stripObj = (
  ...
  if (attributes) {
      ...
      for (let i = 0; i < arrAttr.length; i++) {
        ...
        if (Object.prototype.hasOwnProperty.call(obj, attr[0])) {
          if (attr.length === 1) ret[attr[0]] = obj[attr[0]];
            ...
            Object.prototype.hasOwnProperty.call(obj[attr[0]], attr[1])
          ) {
            // name.familyName
            if (!ret[attr[0]]) ret[attr[0]] = {};
            ret[attr[0]][attr[1]] = obj[attr[0]][attr[1]];
          } else if (Array.isArray(obj[attr[0]])) {
            // emails.value / phoneNumbers.type
            if (!ret[attr[0]]) ret[attr[0]] = [];
            ...
            for (let j = 0; j < arr.length; j++) {
              if (typeof arr[j] !== "object") {
                ...
                Object.prototype.hasOwnProperty.call(arr[j], attr[1])
              ) {
                if (ret[attr[0]].length !== arr.length) {
                  // initiate
                  for (let i = 0; i < arr.length; i++) ret[attr[0]].push({}); // need arrCheckEmpty
                }
                ret[attr[0]][j][attr[1]] = arr[j][attr[1]];
                if (!arrCheckEmpty.includes(attr[0]))
                  arrCheckEmpty.push(attr[0]);
              }
            }
          }
        }
    ...
    const arrAttr = excludedAttributes
      .split(",")
      .filter(Boolean)
      .map((item) => item.trim())
    ...
    arrRet = arrObj.map((obj) => {
      ...
      for (let i = 0; i < arrAttr.length; i++) {
        ...
        if (Object.prototype.hasOwnProperty.call(ret, attr[0])) {
          ...
          else if (Object.prototype.hasOwnProperty.call(ret[attr[0]], attr[1]))
            delete ret[attr[0]][attr[1]]; // name.familyName
          else if (Array.isArray(ret[attr[0]])) {
            // emails.value / phoneNumbers.type
            const arr = ret[attr[0]];
            for (let j = 0; j < arr.length; j++) {
              if (Object.prototype.hasOwnProperty.call(arr[j], attr[1])) {
                const index = arr.findIndex((el: Record<string, any>) =>
                  Object.prototype.hasOwnProperty.call(el, attr[1])
                );
                if (index > -1) {
                  delete arr[index][attr[1]];
                  try {
                    if (JSON.stringify(arr[index]) === "{}")
                      arr.splice(index, 1);
                  } catch (err) {
                    return;
                  }
...
export const sortByKey = (key: string, order: string = "ascending") => {
  return (a: any, b: any) => {
    ...
    if (
      !Object.prototype.hasOwnProperty.call(a, levels[0]) ||
      !Object.prototype.hasOwnProperty.call(b, levels[0])
    )
      return 0;
...
export const getEncrypted = function (pw: string, seed: string) {
  ...
  try {
    ...
    // password considered as cleartext and needs to be encrypted
    const pwclear = pw;
    const iv = crypto.randomBytes(ivLength);
    const cipher = crypto.createCipheriv(
      "aes-128-cbc",
...
export const createRandomPassword = function (len: number, ...set: string[]) {
  ...
  let res;
  if (len > 3 && set.length === 0) {
    // ensure all variants are included: lower, upper, numeric and special
    res = Object.assign(
 ...
 */
export const formUrlEncodedToJSON = function (
  body?: string
): Record<string, any> {
  if (!body) return {};
 ...
 * @returns json formatted body
 */
export const formDataMultipartToJSON = function (
  body?: string,
  boundary?: string
): Record<string, any> {
  if (!body) return {}; // --delimiter123\nContent-Disposition: form-data; name="field1"\n\nvalue1\n--delimiter123\nContent-Disposition: form-data; name="field2"; filename="example.txt"\n\nvalue2
  if (!boundary) {
    // if boundary is missing, try to infer it
 ...
 */
export const getBase64CertificateThumbprint = function (
  pemCertContent: string,
  shaVersion: "sha1" | "sha256" = "sha1"
): string {
  if (!pemCertContent) return "";
  const certMatch = pemCertContent.match(
...
const getOrganizationalUnit = (objectId: string) => {
  ...
  // throw new Error("ou is not defined for creating version in getEtag function")
};
...
export const getEtag = function (obj: ObjWithId): string {
